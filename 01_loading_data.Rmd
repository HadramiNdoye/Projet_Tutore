...
Le premier graphique illustre le logarithme de l’expression des gènes en fonction d’un échantillon de 223 individus. Ces individus sont triés par la valeur d’expression croissante, ce qui se voit sur la courbe : la courbe est croissante.
...
**ATTENTION!!** ici il faut dire et donner la commande (comb-p) que vous avez lancé pour obtenir la (les) ficheirs de sortie.

**TODO** De la même que vous avez afficher *pval.slk*, il faut afficher *pval.slk.fdr* lu dans le fichier `XXX.fdr.bed.gz`. Il n’y a pas besoind e lire le ficheir `XXX.slk.bed.gz` car le ficheir ``XXX.fdr.bed.gz` contient la mem info avec la colonne FDR en plus.
...
plot(-log10(pval.ewas),col="red",pch=19,main = "SGCE, p-valeurs SLK vs p-valeurs de la regression", ylim=c(0, max(-log10(pval.slk), -log10(pval.ewas))))
points(-log10(pval.slk),col="blue",pch=18)
# Ajouter une légende
legend("topright", legend=c("pvalues lm", "pvalues comb-p (slk)", "pvalues comb-p (fdr)"), col = c("red","blue", "purple"),pch= c(19,18))
```
...
**ATTENTION** Reprendre la partie suivamnte : 
